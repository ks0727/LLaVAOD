...
def eval_model(args):
	...
	all_acc = []
	question_idx = 0
	support_prompt_json_path = "/po4/ksakai/src/LLaVAOD/result.json"

	with open(support_prompt_json_path,"r") as f:
		support_dic = json.load(f)
	...
	prompt_ready = "You will be given visual information that consists of object label and object bounding boxes. If the information is not helpfull, you don't have to use it. By utilizing the given information, answer the question.\n  Visual information :"
	for test_type in ['direct_attributes', 'relative_position']:
		...
		for image_file in tqdm(image_files):
			...
			support_prompt = support_dic[test_type+'/'+image_file]
			question = annotation['question']
			if support_prompt is not None:
				question = prompt_ready + support_prompt + question
